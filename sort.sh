...
# read overrides
#if [[ "$INPUT" =~ show.(.*)[$\ ] ]]; then SHOW="${BASH_REMATCH[1]}"; sendToLog "Show overridden to $SHOW"; fi
#if [[ "$INPUT" =~ type.(.*)[$\ ] ]]; then TYPE="${BASH_REMATCH[1]}"; sendToLog "Type overridden to $TYPE"; fi
#if [[ "$INPUT" =~ quality.(.*)[$\ ] ]]; then QUALITY="${BASH_REMATCH[1]}"; sendToLog "Quality overridden to $QUALITY"; fi
#echo $SHOW
#echo $TYPE
#echo $QUALITY
